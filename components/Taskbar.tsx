...
export default function Taskbar() {
  ...
  useEffect(() => {
    ...
    if (!dockAutoHide) return () => clearTimeout(showTimeout);
  ...
  return (
    <div
      ...
      style={{
        ...
        backdropFilter:       dockStyle !== 'transparent' ? 'blur(var(--glass-blur)) saturate(var(--glass-saturate))' : 'none',
        WebkitBackdropFilter: dockStyle !== 'transparent' ? 'blur(var(--glass-blur)) saturate(var(--glass-saturate))' : 'none',
        border: getDockBorder(),
        boxShadow: dockStyle === 'transparent' ? 'none' : isBottom ? '0 -6px 30px rgba(0,0,0,0.35)' : '0 6px 30px rgba(0,0,0,0.35)',
        display: 'flex',
        alignItems: 'center',
        ...
        zIndex: 999999,
        userSelect: 'none',
        transform: hidden ? `translateY(${isBottom ? '110%' : '-110%'})` : 'translateY(0)',
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', minWidth: 120 }}>
        <button
          ...
          style={{
            ...
            transition: 'background var(--dur-fast), transform var(--dur-fast) var(--ease-spring), box-shadow var(--dur-normal)',
            boxSizing: 'border-box',
          }}
          onMouseEnter={(e) => {
        ...
        >
          {/* Inline SVG fixes path resolution bugs entirely */}
          <svg 
            viewBox="0 0 24 24" 
            fill="none" 
            stroke="currentColor" 
            ...
            style={{ width: '55%', height: '55%', color: 'inherit' }}
          >
      ...
      <div
        style={{
          ...
          alignItems: 'center',
          gap: 'var(--dock-gap)',
          background: dockStyle === 'pill' ? 'var(--glass-bg)' : 'rgba(255,255,255,0.02)',
          padding:    dockStyle === 'pill' ? 'var(--dock-padding) var(--dock-padding)' : '0px 0px',
          borderRadius: dockStyle === 'pill' ? 'var(--radius-full)' : '6px',
          border: '1px solid var(--glass-border)',
          boxShadow: dockStyle === 'pill' ? 'inset 0 1px 0 rgba(255,255,255,0.05)' : 'none',
        }}
      >
        {systemApps.map((app) => {
          ...
          const imgSrc      = iconImages[app.id] ?? app.icon;
          ...
          return (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                position: 'relative',
                background: isCurrent ? `${app.color}18` : 'transparent',
                border:     isCurrent ? `1px solid ${app.color}33` : '1px solid transparent',
                boxShadow:  isCurrent ? `0 0 14px ${app.color}22` : 'none',
                transition: 'transform var(--dur-fast) var(--ease-spring), background var(--dur-fast), box-shadow var(--dur-normal)',
            ...
            >
              <img
                ...
                style={{
                  ...
                  height: '54%',
                  objectFit: 'contain',
                  filter: isCurrent ? `drop-shadow(0 2px 8px ${app.color}88)` : undefined,
              ...
              {isRunning && (
                <div
                  style={{
                    ...
                    height: 3,
                    borderRadius: 99,
                    background: isCurrent ? app.color : 'rgba(255,255,255,0.4)',
                    boxShadow:  isCurrent ? `0 0 8px ${app.color}` : 'none',
